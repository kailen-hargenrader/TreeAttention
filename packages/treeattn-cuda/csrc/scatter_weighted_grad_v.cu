...
void launch_scatter_weighted_grad_v_forward_kernel(
  ...
  int blocks,
  int threads) {
  scatter_weighted_grad_v_forward_kernel<index_t><<<
    blocks,
    threads,
    0,
...
torch::Tensor scatter_weighted_grad_v_forward_cuda(
  ...
  if (sampled_indices.scalar_type() == torch::kInt32) {
    launch_scatter_weighted_grad_v_forward_kernel<int32_t>(
        grad_output,
        sampled_indices,
        attn_weights,
        grad_v,
    ...
    launch_scatter_weighted_grad_v_forward_kernel<int64_t>(
        grad_output,
        sampled_indices,
        attn_weights,
        grad_v,
